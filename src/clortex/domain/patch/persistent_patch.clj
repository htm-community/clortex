  ...
  (let [conn (:conn ctx)
        ...
        tx-tuples (for [i (range n)
                    ...
                    [{:db/id input-id
                      :neuron/index index
                      :neuron/active? false}
                     {:db/id patch-id :patch/inputs input-id}])
        tx-data (vec (apply concat tx-tuples))]
...
(defn set-input-bits
  [ctx patch-uuid sdr]
  (let [conn (:conn ctx)
        current-inputs (input-values ctx patch-uuid)
